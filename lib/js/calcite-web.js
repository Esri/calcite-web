...
calcite.dropdown = function () {
  ...
  function toggleDropdown (dropdown) {
    ...
    if (isActive) {
      ...
      calcite.dom.addEvent(body, calcite.dom.event(), bodyClickHandler);
    }
  }

  function bodyClickHandler (event) {
    console.log(event);
    // var dropdown = calcite.dom.closest('js-dropdown', calcite.dom.eventTarget(event));
    // if (!dropdown) {
    //   closeAllDropdowns();
    //   calcite.dom.removeEvent(body, calcite.dom.event(), bodyClickHandler);
    // }
  }
  ...
  function bindDropdown (toggle) {
    calcite.dom.addEvent(toggle, calcite.dom.event(), function(event) {
      ...
      var dropdown = parent.querySelectorAll('.js-dropdown')[0];
