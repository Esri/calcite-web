  ...
  calcite.filterDropdown = function (domNode) {
    ...
    dropdowns.forEach(function (dropdown) {
      ...
      function showActive() {
        ...
        for (i = 0; i < items.length; i++) {
          ...
          if (hasClass(item, 'is-active')) {
            var template = '<span class="filter-dropdown-active">' + item.innerHTML  + '<a class="filter-dropdown-remove" href="#"></a></span>';
      ...
      function clearItem (e) {
        e.preventDefault();
        var target = e.target.parentNode.innerHTML.split('<')[0];
        for (i = 0; i < items.length; i++) {
          if (target == items[i].innerHTML) {
      ...
      function toggleDropdown (e) {
        var isExpanded = list.getAttribute('aria-expanded');
        if (isExpanded == 'true') {
          list.setAttribute('aria-expanded', false);
        } else {
