(function Calcite () {
  ...
  // prevents focus from moving outside a dom node
  function fenceFocus (domNode, e) {
    console.log(domNode, e);
    // lsiten to focus event
    // if focus target is not child of domnode,
    // set focus to dom node
  }
  ...
  calcite.drawer = function (domNode) {
    ...
    drawers.forEach(function (drawer) {
      addEvent(document, 'keyup', function(e) {
        if (hasClass(drawer, 'is-active') && e.keyCode == 27) {
          toggleActive(drawers, drawer);
          toggleAriaHidden([wrapper, footer]);
        }
      });
  ...
  calcite.modal = function (domNode) {
    ...
    function escapeClose (e) {
      ...
      if (e.keyCode === 27) {
        ...
        toggleAriaHidden([wrapper, footer]);
        removeEvent(document, 'keyup', escapeClose);
      }
    }
    ...
    function bindToggle (e) {
      ...
      if (isOpen) {
        ...
        addEvent(document, 'keyup', escapeClose);
    ...
    toggles.forEach(function (toggle) {
      addEvent(toggle, click(), bindToggle);
    });
