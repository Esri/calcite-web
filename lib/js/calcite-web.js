(function Calcite () {
  ...
  calcite.filterDropdown = function (domNode) {
    ...
    dropdowns.forEach(function (dropdown) {
      ...
      nodeListToArray(dropdown.querySelectorAll('.filter-dropdown-link')).forEach(function (filter) {
        items.push(filter);
        if (hasClass(filter, 'is-active')) {
          selectedItems.push(filter);
        }
        addEvent(filter, 'click', toggleFilter);
      });

      function toggleDropdown (e) {
        toggleActive([list], list);
      }
      ...
      function listSelected () {
        selectedItems.forEach(function (item) {
          var selectedItem = '<a href="#" class="filter-dropdown-active-link label">' + item.innerHTML + '</a>';
          container.insertAdjacentHTML('afterend', selectedItem);
        });
      }

      function clearAllSelected (e) {
        if (e) e.preventDefault();
        var activeList = dropdown.querySelectorAll('.filter-dropdown-active-link');
        for (i = 0; i < activeList.length; i++) {
          activeList[i].parentNode.removeChild(activeList[i]);
        }
        var list = dropdown.querySelectorAll('.filter-dropdown-link');
        console.log(list);
        for (i = 0; i < list.length; i++) {
          console.log(list[i]);
          removeClass(list[i], 'is-active');
        }
        selectedItems = [];
      }
      ...
      addEvent(input, 'keyup', function(e) {
        ...
        calcite.filterList(input.value, items).forEach(function(item) {
          removeClass(item, 'is-hidden');
        });
      });

      addEvent(container, 'focusin', toggleDropdown);
      addEvent(container, 'focusout', toggleDropdown);
      addEvent(clearButton, 'click', clearAllSelected);
