/* calcite-web - v0.13.0 - 2015-09-10
...
(function Calcite () {
  ...
  calcite.filterDropdown = function (domNode) {
    ...
    dropdowns.forEach(function (dropdown) {
      ...
      function showActive() {
        var activeItems = dropdown.querySelectorAll('.filter-dropdown-active-link');
        ...
        for (i = 0; i < items.length; i++) {
          ...
          if (hasClass(item, 'is-active')) {
            var template = '<a href="#" class="filter-dropdown-active-link label">' + item.innerHTML  + '</a>';
            container.insertAdjacentHTML('afterend', template);
          }
        }

        activeItems = dropdown.querySelectorAll('.filter-dropdown-active-link');

        for (i = 0; i < activeItems.length; i++) {
          addEvent(activeItems[i], 'click', clearItem);
        }
      }
